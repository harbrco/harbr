...
(function( $ ) {
  ...
  $.fn.blockScroll = function(options) {
    ...
    if(settings.triggerRatio > settings.paddingRatio*0.95) { settings.triggerRatio = settings.paddingRatio*0.95; }

    var theDivs = this.children().filter("article");
    ...
    function gotoDiv(number)
    {
      ...
      $('html, body').animate({scrollTop: $(theDivs[activeDiv]).offset().top}, settings.scrollDuration, 'swing', function() {
        $(theDivs[activeDiv]).animate({opacity: 1}, settings.fadeDuration,'linear', function() {
          setTimeout(function(){
